...
if __name__ == "__main__":
	   ...
	   #####################################################################
		...
		clf=[]
                #####################################################################
                with open('./Classifier_Models/All_Dialects_MLP_Model.pkl', 'rb') as fid:
		...
		labels=clf.classes_
                Old_Five_Dilects_Egyptian=classesResult[0][0]*100
                Old_Five_Dilects_Gulf=classesResult[0][1]*100
                Old_Five_Dilects_Iraqi=classesResult[0][2]*100
                Old_Five_Dilects_Levant=classesResult[0][3]*100
                Old_Five_Dilects_Maghrebi=classesResult[0][4]*100
                ############################################################################################################################################################  
                ############################################################################################################################################################
                ############################################################################################################################################################
                ############################################################################################################################################################
		...
		clf=[]
                #####################################################################
                with open('./Classifier_Models/Egypt_MLP_Model.pkl', 'rb') as fid:
		           clf = cPickle.load(fid)
		line_features=FindFeatureVectorForInputText(text,features_vector)
                result_label=clf.predict([line_features])
		classesResult=clf.predict_proba([line_features]) 
		...
		clf=[]
                #####################################################################
                with open('./Classifier_Models/Gulf_MLP_Model.pkl', 'rb') as fid:
		           clf = cPickle.load(fid)
                result_label=clf.predict([line_features])
		classesResult=clf.predict_proba([line_features]) 
		labels=clf.classes_
		...
		clf=[]
                #####################################################################
                with open('./Classifier_Models/Iraqi_MLP_Model.pkl', 'rb') as fid:
		           clf = cPickle.load(fid)
                result_label=clf.predict([line_features])
		classesResult=clf.predict_proba([line_features]) 
		labels=clf.classes_
		...
		clf=[]
                #####################################################################
                with open('./Classifier_Models/Levant_MLP_Model.pkl', 'rb') as fid:
		           clf = cPickle.load(fid)
                result_label=clf.predict([line_features])
		classesResult=clf.predict_proba([line_features]) 
		labels=clf.classes_
		...
		clf=[]
                #####################################################################
                with open('./Classifier_Models/Maghreb_MLP_Model.pkl', 'rb') as fid:
		           clf = cPickle.load(fid)
                result_label=clf.predict([line_features])
		classesResult=clf.predict_proba([line_features]) 
		labels=clf.classes_
		...
		clf=[]
                #####################################################################
                with open('./Classifier_Models/Six_MLP_Model_3.pkl', 'rb') as fid:
		           clf = cPickle.load(fid)
		...
		labels=clf.classes_
                ...
                ############################################################################################################################################################
                ############################################################################################################################################################
                ####################################################                3-grams ModernStandard/Non_ModernStandard      #########################################
                ############################################################################################################################################################
                ############################################################################################################################################################
                ############################################################################################################################################################
                ############################################################################################################################################################
		...
		clf=[]
                #####################################################################
                with open('./Classifier_Models/Six_MLP_Model_2.pkl', 'rb') as fid:
		           clf = cPickle.load(fid)

                #####################################################################

		line_features=FindFeatureVectorForInputText(text,Features_Vector_93k)
		result_label=clf.predict([line_features])
		classesResult=clf.predict_proba([line_features]) 
		labels=clf.classes_
                Three_gram_Modern=classesResult[0][0]*100
                Three_gram_Not_Modern=classesResult[0][1]*100
                ...
                if choiceNumber in '1':
                               ...
                               ModernStandard_Result=max(Three_gram_Modern,Two_gram_Six_Dilects_ModernStandard)
                               Other_Result=other
                               if (Other_Result>100):
                ...
                ############################################################################################################################################################
                        ...
                        if max_Egyptian > 95:
                             egyptian_result=max_Egyptian-(Three_gram_Modern/2+max_Gulf+max_Iraqi+max_Levant+max_Maghrebi)
                        ...
                        if max_Gulf > 95:
                             gulf_result=max_Gulf-(Three_gram_Modern/2+max_Egyptian+max_Iraqi+max_Levant+max_Maghrebi)
			     ...
			     gulf_result=max_Gulf
                        ...
                        if max_Iraqi > 95:
		             iraqi_result=max_Iraqi-(Three_gram_Modern/2+max_Gulf+max_Egyptian+max_Levant+max_Maghrebi)
			     ...
			     iraqi_result=max_Iraqi
                        ...
                        if max_Levant > 95:
		             levant_result=max_Levant-(Three_gram_Modern/2+max_Gulf+max_Iraqi+max_Egyptian+max_Maghrebi)
			     ...
			     levant_result=max_Levant
                        ...
                        if max_Maghrebi > 95:
		             maghrebi_result=max_Maghrebi-(Three_gram_Modern/2+max_Gulf+max_Iraqi+max_Levant+max_Egyptian)
